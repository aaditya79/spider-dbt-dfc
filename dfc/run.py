...
from spider_agent.agent.harbor_trajectory import write_harbor_trajectory
...
def config() -> argparse.Namespace:
    ...
    parser.add_argument("--max_tokens", type=int, default=8192)  # 2500 truncated large CreateFile bodies mid-SQL
    ...
    # DFC scoped policy: when set to "recharge001", run the post-materialization
    # discount-amount checker and steer the agent on violation. Only affects that task.
    parser.add_argument("--dfc_policy", type=str, default=None)
...
def test(
    ...
    for task_config in task_configs:
        ...
        spider_result = {"finished": done, "steps": len(trajectory["trajectory"]),
                           "result": result_output,"result_files": result_files,
                           "dfc_policy": args.dfc_policy,
                           "dfc_retries": getattr(agent, "dfc_retries", 0),
                           "dfc_events": getattr(agent, "dfc_events", []),
                           **trajectory}
        with open(os.path.join(output_dir, "spider/result.json"), "w") as f:
            json.dump(spider_result, f, indent=2)

        # ALSO emit a harbor parity-experiments conforming trajectory (for cross-run
        # comparison with the team's GPT-5.5 runs). Native result.json above is
        # unchanged; this drops the non-harbor `steps`/`result_files` keys.
        harbor_path = write_harbor_trajectory(output_dir, trajectory, done, result_output)
        logger.info("Wrote harbor trajectory: %s", harbor_path)
